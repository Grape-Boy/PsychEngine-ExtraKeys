...
class NotesSubState extends MusicBeatSubstate
{
	...
	public function new() {
		...
		for (i in 0...ClientPrefs.arrowHSV.length) {
			...
			for (j in 0...3) {
				var optionText:Alphabet = new Alphabet(posX + (225 * j) + 250, yPos + 45, Std.string(ClientPrefs.arrowHSV[i][j]), true);
		...
		hsbText = new Alphabet(posX + 560, -115, "Hue      Saturation  Brightness", false);
		hsbText.scaleX = 0.6;
		hsbText.scaleY = 0.6;
		add(hsbText);
